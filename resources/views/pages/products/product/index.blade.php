...
@section('pages')
    ...
    <div class="px-5 md:px-20 pt-10">
        ...
        <div class="text-[#89B441] text-[18px] pt-20 font-normal flex items-center gap-x-2">
            ...
            <a href="/sellers/{{ $umkm_all_detail['slug-umkm'] }}">{{ $umkm_all_detail['product']['name'] }}</a>
            <svg width="8" height="15" viewBox="0 0 8 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M1 1.32257L7 7.51612L1 13.7097" stroke="black" stroke-opacity="0.5"/>
            </svg>
                        ...
                        <div class="lg:flex gap-x-10">
                            <div class="flex flex-col md:w-full lg:w-2/6 gap-y-4 ">
                                ...
                                <div class="flex pt-2 gap-x-4">
                                    <div class="w-14 h-14">
                                    <img src={{ "https://api.andamantau.com/".($umkm_all_detail['umkm']['umkm_image']) }} alt="">
                                    </div>
                        ...
                        {{-- @foreach ($seller['products'] as $product) --}}
                            {{-- @if ($product['slug'] !== $productData['slug']) --}}
                                @component('components.product-card')
                                    @slot('productImage')
                                        {{ "https://api.andamantau.com/".($product['image']) }}
                                    @endslot
                                    @slot('productUrl')
                                        /sellers/{{ $umkm_all_detail['slug-umkm'] }}/products/{{$umkm_all_detail['slug-product'] }}
                                    @endslot
                                    @slot('productTitle')
                                        {{ $product['name'] }}
                                    @endslot
